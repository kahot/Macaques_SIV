...
#create.dir("Output/SeqData/")

###########################
#
SIVFiles<-list.files("Output/CSV/",pattern="csv")

sfreqPatTs<-data.frame(row.names=SIVFiles)
...
coding.start<-238
coding.end<-238+572
no<-data.frame("pos"=c(238:810))
for (i in 1:length(SIVFiles)){
        print(i)
        id<-substr(paste(SIVFiles[i]),start=1,stop=3)
        print(id)
        ...
        write.csv(Mutation.freq.filtered, paste0("Output/",id,"_mut.freq.filtered.csv"))
}
